...
import { ChangeParams, User } from '@/types'
...
const SingleAssigneeSelect: FC<SingleAssigneeSelectProps> = ({ assignee, onChange, size = 'default' }) => {
	...
	const handleSelect = (id: string) => {
		...
		if (newAssignee) {
			...
			if (onChange)
				onChange({ newValue: [newAssignee], previousValue: selectedAssignee ? [selectedAssignee] : [] })
		}
		setOpen(false)
	}
					...
					<TooltipTrigger asChild>
						<PopoverTrigger asChild>
							<Button
								...
								aria-expanded={open}
								className={cn(
									'h-10 p-0 px-2 flex items-center justify-start bg-inherit hover:bg-inherit',
									{
...
export const MultiAssigneeSelect: FC<MultiAssigneeSelecProps> = ({ assignee, onChange, size = 'default' }) => {
	...
	const toggleUser = (user: User) => {
		const newUsers = selectedUsers.some((u) => u.id === user.id)
				...
				>
					{selectedUsers.length > 0 ? (
						<div className='flex items-center'>
							{(isHovering ? selectedUsers : selectedUsers.slice(0, NUMBER_OF_USERS_TO_SHOW)).map(
								(user, index) => (
