...
import { InsertTaskData, TaskProperty, TaskPropertyValue, TaskUserRelationMinimal, User } from '@/types'
...
const SelectProjectFormItem = forwardRef<
	FormItemContentRefValue<FormItemValue['project']>,
	{ project?: Project; onChange: (project: Project) => void }
	...
	function SelectPriorityFormItem({ priority: initialPriority }, ref) {
		...
		return (
			<div className='relative'>
				...
				<PrioritySelect size='full' priority={priority} onChange={setPriority} />
			</div>
		)
	}
)

const SelectStatusFormItem = forwardRef<
	FormItemContentRefValue<FormItemValue['status']>,
	{ status?: WorkflowStatus; allStatus: WorkflowStatus[]; isLoading: boolean }
...
const SelectAssigneeFormItem = forwardRef<
	FormItemContentRefValue<FormItemValue['assignee']>,
	{ singleUser: boolean; assignee?: User[] }
>(function SelectStatusFormItem({ singleUser, assignee: initialAssignee }, ref) {
...
const CustomPropertyFormItem = forwardRef<
	DynamicFormItemContentRefValue<TaskPropertyValue>,
	ComponentProps<typeof CustomProperty>
>(function CustomPropertyFormItem(props, ref) {
	...
	return (
		<TooltipProvider>
			<Tooltip open={showError}>
				<CustomProperty asTooltipTrigger size='full' {...props} value={value} onChange={setValue} />
...
const CustomUserPropertyFormItem = forwardRef<
	DynamicFormItemContentRefValue<TaskPropertyValue>,
	ComponentProps<typeof CustomUserProperty>
>(function CustomPropertyFormItem(props, ref) {
	...
	return (
		<TooltipProvider>
			<Tooltip open={showError}>
				<CustomUserProperty asTooltipTrigger size='full' {...props} value={users} onChange={setUsers} />
				<TooltipContent align='start' sideOffset={20}>
					<p className='text-xs p-1 bg-destructive text-foreground rounded-md'>{props.name} is requried</p>
				</TooltipContent>
			</Tooltip>
