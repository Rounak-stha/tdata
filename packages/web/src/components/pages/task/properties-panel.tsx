...
export const PropertiesPanel: FC<PropertiesPanelProps> = ({ task }) => {
	...
	const handleStandardFieldPrimitiveValueUpdate = async (
		change: ChangeParams<string>,
		name: TaskStandardFieldUpdateKeys
	) => {
		await updateTask(
			{ id: task.id, organizationId: task.organizationId },
			{
	...
	const handleUserRelationUpdate = async (change: ChangeParams<User[]>, name: string) => {
		...
		await updateTask(
			{ id: task.id, organizationId: task.organizationId },
			{
				...
				data: {
					name,
					newUserId: action == 'add' ? diff[0].id : undefined,
					previousUserId: action == 'remove' ? diff[0].id : undefined
				}
			}
		)
	}
		...
		<div className='space-y-6'>
			...
			<div className='space-y-4'>
				<div className='space-y-1'>
					...
					<StatusSelect
						size='full'
						allStatus={task.projectTemplate.workflow.statuses}
