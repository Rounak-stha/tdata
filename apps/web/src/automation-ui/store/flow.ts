...
import { getPlaceholderNodeAndedges, isValidConnection, createNode, createEdge } from "@/automation-ui/utils";
import type { AppState, FlowVariable } from "@/automation-ui/types";
import { Automation, FlowEdge, FlowNode, NodeType, ProjectDetail } from "@tdata/shared/types";
...
export const useFlowStore = create<AppState>((set, get) => ({
  ...
  onSelectionChange: ({ nodes, edges }) => {
    ...
    set({ selectedElements: { nodes: flowNodes, edges } });
  },
  ...
  replaceNode: (nodeId, newNodeType) => {
    ...
    const edgeToReplace = edges.find((edge) => edge.target === nodeId);

    if (!nodeToReplace || !edgeToReplace) return;
    ...
    const newNode: FlowNode = createNode(newNodeType, parentNode, { label: newNodeType === "ActionNode" ? "Action" : "Condition" });
    newNode.position = nodeToReplace.position;

    const newEdge: FlowEdge = createEdge(parentNode, newNode, edgeToReplace.sourceHandle || undefined);
...
export const createFlowStore = (initialAutomation?: Automation | null) => {
  const nodes = initialAutomation?.flow.nodes || InitialNodes;
  const edges = initialAutomation?.flow.edges || InitialEdges;
