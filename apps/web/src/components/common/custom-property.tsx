...
import { TaskPropertyValue, User } from "@tdata/shared/types";
import { TemplateProperty, ChangeParams } from "@types";
...
const TaskCustomSelectProperty: FC<TaskCustomPropertyProps> = ({ type, name, options, value, size, onChange }) => {
  if (type == "select" || type == "multiSelect") {
    return (
      <div>
        <Label className="text-muted-foreground">{name}</Label>
        <CustomValueSelect size={size} options={options || ["Invalid Property"]} value={value} onChange={onChange} multiSelect={type == "multiSelect"} />
      </div>
    );
  }
...
const TaskCustomInputPropertyAsTtTrigger: FC<TaskCustomInputPropertyAsTtTriggerProps> = ({ type, name, value: initialValue, onChange }) => {
  const [value, setValue] = useState(initialValue || "");
  const { debouncedValue, previousValue } = useDebounce(value, DebounceDelay);
  const initialRender = useRef<boolean>(true);
...
const TaskCustomSelectPropertyAsTTTrigger: FC<TaskCustomPropertyProps> = ({ type, name, options, value, size, onChange }) => {
  if (type == "select" || type == "multiSelect") {
    return (
      <div>
        <Label className="text-muted-foreground">{name}</Label>
...
const TaskCustomUserPropertyAsTtTrigger: FC<TaskCustomUserPropertyProps> = ({ name, value, size, onChange, singleUser }) => {
  return (
    <div>
      <Label className="text-muted-foreground">{name}</Label>
      <TooltipTrigger asChild>
