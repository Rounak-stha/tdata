...
import type { TaskGroupedByStatus } from "@tdata/shared/types";

type KanbanBoardProps = {
  tasks: TaskGroupedByStatus[];
};

export const KanbanBoard: FC<KanbanBoardProps> = ({ tasks }) => {
  return (
    <div className="flex space-y-8">
      ...
      <ScrollArea className="pb-6 w-1 flex-1">
        <div className="flex gap-4">
          {tasks.map((t) => {
            return <BoardColumn key={t.group.id} status={t.group} tasks={t.tasks} onTaskUpdate={() => {}} />;
          })}
        </div>
