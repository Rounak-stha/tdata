...
import { TaskPropertyValue } from "@tdata/shared/types";
...
export function SingleValueSelect({ value: initialValue, options, onChange, size = "default", isLoading }: SingleSelectProps) {
...
export function MultiSelect({ value: initialValue, options, onChange }: MultiSelectProps) {
  ...
  const handleSelect = (option: string) => {
    const updatedSelection = selected.some((item) => item === option) ? selected.filter((item) => item !== option) : [...selected, option];
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button variant="outline" role="combobox" aria-expanded={open} className="w-full h-10 justify-between bg-inherit hover:bg-inherit px-2">
            ...
            <CommandGroup>
              {options.map((option) => (
                <CommandItem key={option} onSelect={() => handleSelect(option)}>
                  <CheckIcon className={cn("mr-2 h-4 w-4", selected.some((item) => item === option) ? "opacity-100" : "opacity-0")} />
                  {option}
                </CommandItem>
              ))}
            </CommandGroup>
