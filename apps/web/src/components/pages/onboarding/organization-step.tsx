  ...
  return (
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground">To start using TData, you need to create or join an Organization</p>
      </div>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center space-x-2">
            ...
            <div className="relative flex-1">
              <Input id="organizationKey" value={organizationKey} onChange={(e) => setOrganizationKey(e.target.value.toUpperCase())} placeholder="Organization Key" required />
              {isCheckingUrl && (
              ...
              {!isCheckingUrl && isKeyAvailable !== null && (
                <div className="absolute inset-y-0 right-0 flex items-center pr-3">
                  {isKeyAvailable ? <CheckCircleIcon className="h-5 w-5 text-primary" /> : <XCircleIcon className="h-5 w-5 text-red-500" />}
                </div>
              )}
            </div>
          </div>
          <p className="text-xs text-muted-foreground">
            {isKeyAvailable === false ? "This workspace URL is already taken. Please choose another." : "You can only edit the slug of the URL"}
          </p>
        </div>
